...
#' @param seq: This variable will get the sequence of the methods to be used. DEFAULT to (1,2,3,4)
                               ...
                               overall_MAD_cutoff=1.5, overall_MAD_mad_const=1.4826, overall_MAD_replace=NA)
{
  ...
  if(sum(ifelse(grepl( '[0-4]', seq),0,1))>0)
  {
    stop("In sequence number 1 to 4 is acceptable.
         1. Decompose method,
         2. Rolling quarters using MAD,
         3. Rolling quarters without NAs using MAD and
         4. Overall MAD")
  }
  else
  { 
